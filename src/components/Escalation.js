...
const Escalation = () => {
  const [activePage, setActivePage] = useState("escalation");
  ...
  const handleSubmit = (e) => {
    ...
    const existingReports = JSON.parse(localStorage.getItem("escalationReports")) || [];

    // Add new report to array
    const updatedReports = [...existingReports, formData];

    // Save updated array to localStorage
    localStorage.setItem("escalationReports", JSON.stringify(updatedReports));
